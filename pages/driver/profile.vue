<template>
  <div class="container mx-auto px-4 py-8">
      ...
      <UCard>
        <div class="flex items-center space-x-4">
          <div class="p-3 bg-primary/10 rounded-full">
            <Icon name="i-lucide-steering-wheel" class="w-8 h-8 text-primary" />
          </div>
          <div class="flex-1">
            <h2 class="text-xl font-semibold">{{ user?.name || 'Driver' }}</h2>
            <p class="text-gray-500">Driver Account</p>
          </div>
        ...
        <div v-if="vehicle" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <p class="mt-1">{{ vehicle.vehicle_number }}</p>
            </div>
            <div>
    ...
    <!-- Vehicle Modal -->
    <UModal v-model="showVehicleModal">
      <UCard>
        <template #header>
...
const fetchVehicle = async () => {
  ...
  try {
    const { data, error } = await supabase
      .from('vehicles')
      .select('id, vehicle_number, license_plate, model, capacity, driver_id, created_at, updated_at')
...
const editVehicle = () => {
  if (vehicle.value) {
    Object.assign(vehicleForm, {
      vehicle_number: vehicle.value.vehicle_number,
