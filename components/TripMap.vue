...
const initMap = () => {
  if (!mapContainer.value || !mapboxgl.accessToken) return;

  map = new mapboxgl.Map({
    container: mapContainer.value,
    style: "mapbox://styles/mapbox/streets-v12",
    center: [
      props?.currentLocation?.lng || 0,
  ...
  map.on("load", () => {
    ...
    if (props.destination && mapboxgl.accessToken) {
      updateRoute();
    }
  });
};
...
const updateRoute = async () => {
  if (!map || !props.destination || !mapboxgl.accessToken) return;
